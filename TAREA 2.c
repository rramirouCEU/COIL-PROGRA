...
void corregir_aceleracion(Datos registros[], int total) {                   //TAREA
    /*
    ...
    */
    for (int i = 0; i < total; i++) {                                                   //BUCLE PARA RECORRER LA ESTRUCTURA
        registros[i].linear_acceleration_z = -registros[i].linear_acceleration_z;
    }
}

double calcular_longitud_temporal(int total, double fs) {                   //TAREA : VERIFICACION , CALCULO , DEVOLVER RESULTADO
    /*
        Tarea del estudiante:
    ...
    */
    if (total <= 0 || fs <= 0.0) {                          //VERIFICACION 1.
        return 0.0;
    }

    double lt = total / fs;                         //2.
    return lt;                                      //3.

}

int buscar_indice_primera_sync(const Datos registros[], int total) {        //TAREA : RECORRER HASTA ENCONTRAR UNA CONDICION DESDE EL INICIO
    /*
        Tarea del estudiante:
    ...
    */
    for (int i = 0; i < total; i++) {
        if (registros[i].sync != 0) {           //DONDE SE CUMPLA LA CONDICION DEVUELVE EL VALOR DEL INDICE DONDE SE ENCUENTRA
            return i;
        }
    }
    return -1;
...
int buscar_indice_ultima_sync(const Datos registros[], int total) {         //TAREA : RECORRER HASTA ENCONTRAR UNA CONDICION DESDE EL FINAL
    /*
        Tarea del estudiante:
...
int contar_transiciones_S3_S0(const Datos registros[], int inicio, int fin) {   //TAREA : condicion s3 -> s0
    /*
        Tarea del estudiante:
    ...
    */
    ...
    if (inicio < 0 || fin <= inicio) {              //1. VERIFICACION DE DATOS CORRECTOS
        return 0;
    }


    for (int i = inicio + 1 ; i <= fin ; i++) {     //2. RECORRER "segmentacion_output"
        if (registros[i - 1].segmentation_output == 3 && registros[i].segmentation_output == 0) {   //CONDICION DE PASAR DE 3 a 1 DE SEGUIDO
            pasos++;
        }
    }
...
double calcular_velocidad_marcha(int muestras_sync, double fs) {            //TAREA : VERIFICACION , CALCULO , DEVOLVER RESULTADO
    /*
        Tarea del estudiante:
        ...
        Mientras no este implementada, devuelve 0.0.
    */
    if (muestras_sync <= 0 || fs <= 0.0) {              //MISMA VERIFICACION QUE ANTES
        return 0.0;
    }
    double vm = DISTANCIA_UTIL_10MWT_M * fs / muestras_sync;
    return vm ;
}

double calcular_velocidad_pasos(int pasos, int muestras_pasos, double fs) { //TAREA : VERIFICACION , CALCULO , DEVOLVER RESULTADO
    /*
        Tarea del estudiante:
    ...
    */
    ...
    double vm = pasos * fs / muestras_pasos;
    return vm;
}

double calcular_longitud_zancada(double velocidad_marcha, double velocidad_pasos) {//TAREA : VERIFICACION , CALCULO , DEVOLVER RESULTADO
    /*
        Tarea del estudiante:
   ...
   double lz = velocidad_marcha / velocidad_pasos;
   return lz;
}
